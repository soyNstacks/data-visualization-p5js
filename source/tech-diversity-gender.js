function TechDiversityGender() {
    ...
    this.draw = function() {
        ...
        if (((mouseX - this.layout.leftMargin) / this.layout.plotWidth() < 0.95) && 
            ((mouseX - this.layout.leftMargin) / this.layout.plotWidth() > 0.095)) {
            
            var mouseToPercentage = (mouseX - this.layout.leftMargin) * 100 / this.layout.plotWidth() ;
        } 
        else {
            var mouseToPercentage = 50;
        }
        ...
        for (var i = 0; i < this.data.getRowCount(); i++) {
            ...
            // Change colour of fill according to position of mouse at percentage position
            if (company.female > mouseToPercentage) {
                fill(this.femaleColour);
            } 
            else if (company.female < mouseToPercentage) {
                fill(this.maleColour);
            }
            else if((mouseX < this.layout.leftMargin) 
        ...
        // Mouse interactivity; display data value (in percentage)
        textAlign(CENTER);
        noStroke();
        fill(0);
        text(Math.round(mouseToPercentage) + '%',
             this.mapPercentToWidth(mouseToPercentage) + this.layout.leftMargin,
             this.layout.pad);
        
        // Draw a line that follows mouseX
        stroke(0);
        strokeWeight(1);
        line(this.mapPercentToWidth(mouseToPercentage) + this.layout.leftMargin,
